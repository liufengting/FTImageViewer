...
class FTImageViewer: NSObject , UIScrollViewDelegate,UIGestureRecognizerDelegate{
    ...
    func showImages(images : [String] , atIndex : NSInteger , fromSenderArray: [UIView]){
        ...
        fromRect = UIApplication.sharedApplication().keyWindow?.convertRect(fromSenderArray[atIndex].frame, toView:nil)
...
class FTImageViewBar : UIView {
    ...
    convenience init(frame: CGRect , saveTapBlock: ()->() , closeTapBlock: ()->()) {
        ...
        closeButton.setImage(UIImage(named: closeImagePath), forState: UIControlState.Normal)
        ...
        saveButton.setImage(UIImage(named: saveImagePath), forState: UIControlState.Normal)
